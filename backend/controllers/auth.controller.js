...
// Function to generate an encrypted TOTP secret, auth tag (for GCM) and QR code URL based on email
function generateTotpSecret(email) {
  // Get TOTP secret from otplib
  const secret = otplib.authenticator.generateSecret();
  // Encrypt the secret using AES-GCM
  //const { encryptedSecretCiphertext, encryptedSecretAuthTag } = encrypt(secret);
  ...
  try {
    ...
    if (user && user.role === "user" && user.resetTokenExpiry > Date.now()) {
      ...
      if (isValid) {
        ...
        // Update the user's salt, password and reset token fields in the database
        user.salt = newSalt;
